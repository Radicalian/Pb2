package com.example.user.pb;

import android.app.Activity;
import android.content.Intent;
import android.os.Bundle;
import android.support.v7.app.AppCompatActivity;
import android.view.MenuItem;
import android.view.View;

/**
 * Created by user on 22-07-2017.
 */
public class Intrest extends AppCompatActivity {
    @Override

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.intrest);
        if(getSupportActionBar()!=null){

            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setDisplayShowHomeEnabled(true);

        }



    }

    @Override
    public boolean onOptionsItemSelected(MenuItem item) {
        if(item.getItemId()==android.R.id.home)
            finish();
        return super.onOptionsItemSelected(item);
    }
    public  void onButtonClick28(View v){
        if(v.getId() == R.id.button28){
            Intent i = new Intent(Intrest.this,Deposits.class);
            startActivity(i);
        }
    }
    public  void onButtonClick29(View v){
        if(v.getId() == R.id.button29){
            Intent i = new Intent(Intrest.this,Advances.class);
            startActivity(i);
        }
    }
    public  void onButtonClick30(View v){
        if(v.getId() == R.id.button30){
            Intent i = new Intent(Intrest.this,Services.class);
            startActivity(i);
        }
    }

}
